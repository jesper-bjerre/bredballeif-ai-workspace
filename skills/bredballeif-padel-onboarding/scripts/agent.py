...
def cmd_onboard(bot: HalBookingAutomation, name: str, membership_type: str,
    ...
    # Padel-gruppe eller i Padel Frivillige. Ingen fallback ved API-fejl.
    if not conventus_data:
        print("--- Henter oplysninger fra Conventus ---", flush=True)
        _script_dir = Path(__file__).resolve().parent
        sys.path.insert(0, str(_script_dir))
        try:
            from conventus_agent import (
                PADEL_VOLUNTEERS_GROUP_ID,
                fetch_members,
                resolve_groups,
            )
    ...
    if member_name:
        ...
        if matches:
            ...
            record(
                f"'{member_name}' fundet i Conventus",
                False,
                "ingen match i aktive 2026- eller frivilliggrupper",
            )
