...
```bash
./bin/padel-halbooking.sh search --name "Navn" --detail
./bin/padel-halbooking.sh history --name "Navn"
```
...
Onboarding- og Gmail-notifikationsflowet er også tilgængeligt i CLI'en, men bør beskrives for brugere via `padel-onboarding` skillen, fordi det er et tværsystem-workflow med flere sikkerhedsforudsætninger.
